...
class LexParser:
    def __init__(self, sents, pretrain_model_path='word2vec.pretrain', dim=64):
        self.sents = sents
        self.dim = dim
    ...
    def build_vocab_dict(self):
        index = 1
        d = {}
        for key in self.model.wv.key_to_index:
            d[key] = index
            self.pre_embedding.append(self.model.wv[key])
            index += 1
        return d
    ...
    def get_embedding(self, sig):
        ...
        try:
            if words:
                embedding = sum([self.model.wv[w] for w in words]) / len(words)
