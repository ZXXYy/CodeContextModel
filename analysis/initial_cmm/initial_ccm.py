...
from results_visualize import visualize_count_based_results, visualize_order_based_results, visualize_experience_based_results
...
def generate_initial_seed(test_case, seed_strategy, num_seed=None, percentage=None, step=1, delta_day=None) -> List[tuple]:
    if seed_strategy == "count_based":
        seed_strategy = CountBasedStrategy(percentage=percentage, max_seed_count=MAX_SEED_COUNT)
        ...
        seed_strategy = CountBasedStrategy(seed_count=num_seed, max_seed_count=None)
        graph = XMLTreeParser(os.path.join(test_case, CCM_EXPANDED_GRAPH_FILE))
        initial_seed = seed_strategy.generate_seed(graph)
        return initial_seed
        ...
        seed_strategy = ExperienceBasedStrategy(test_case.split("/")[-1], delta_days=delta_day)
        graph = XMLTreeParser(os.path.join(test_case, CCM_EXPANDED_GRAPH_FILE))
        initial_seed = seed_strategy.generate_seed(graph)
        return initial_seed
        ...
        graph = XMLTreeParser(os.path.join(test_case, CCM_EXPANDED_GRAPH_FILE))
        initial_seed = seed_strategy.generate_seed(graph, step=step)
        return initial_seed
    else:
    ...
    if not os.path.exists(os.path.join(INITIAL_CCM_DIR, "count_based", "result.json")):
        ...
        for percentage in percentage_list:
            ...
            for test_case in tqdm(test_cases):
                initial_seed = generate_initial_seed(test_case, "count_based", percentage=percentage)
                if initial_seed is None:
                    continue
                expanded_ccm = generate_expanded_ccm_from_seed(initial_seed[0], "count_based", test_case, percentage)
                expanded_ccms.append(expanded_ccm)
            dataset_path = build_dataset(expanded_ccms, "count_based", percentage)
            test_hit_rate = inference_dataset(dataset_path, args)
            test_hit_rates[percentage] = test_hit_rate
...
def run_order_based_initial_ccm(test_cases, args):
    if not os.path.exists(os.path.join(INITIAL_CCM_DIR, "order_based", "result.json")):
        ...
        test_hit_rate = inference_dataset(dataset_path, args)
        test_hit_rates["chronology_based"].append(test_hit_rate)
    ...
    if not os.path.exists(os.path.join(INITIAL_CCM_DIR, "experience_based", "result.json")):
        ...
        for delta_day in delta_days:
            ...
            for test_case in tqdm(test_cases):
                ...
                expanded_ccm = generate_expanded_ccm_from_seed(
                    initial_seed[0], "experience_based", test_case, f"{delta_day}_experience_based")
            ...
            experience_based_dataset_path = build_dataset(experience_based_expanded_ccms, "experience_based", f"{delta_day}_experience_based")
            test_hit_rate = inference_dataset(experience_based_dataset_path, args)
            test_hit_rates[f"delta_days_{delta_day}"]["experience_based"] = test_hit_rate
...
def run_step_based_initial_ccm(test_cases, args):
    if not os.path.exists(os.path.join(INITIAL_CCM_DIR, "step_based", "result.json")):
        ...
        for steps in range(1, 4):
            expanded_ccms = []
            for test_case in tqdm(test_cases):
                initial_seed = generate_initial_seed(test_case, "step_based", step=steps)
                if initial_seed is None:
                    continue
